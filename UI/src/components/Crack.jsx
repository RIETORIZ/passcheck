  ...
  return (
    <section className="card span-2" id="crack">
      ...
      <form className="form-grid" onSubmit={onStart}>
        <label>Password to simulate
          <input type="text" value={pwd} onChange={e=>setPwd(e.target.value)} required />
        </label>
        <label>Time limit (sec)
          <input type="number" min="5" max="300" value={timeLimit} onChange={e=>setTimeLimit(e.target.value)} />
        </label>
        <label className="check">
          <input type="checkbox" checked={usePII} onChange={e=>setUsePII(e.target.checked)} /> Use personal info
        </label>
        <div className="row">
