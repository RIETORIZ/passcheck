...
def shannon_entropy_bits(pwd: str) -> float:
    ...
    return round(H * len(pwd), 2)  # total bits ~ bits/char * length
...
def analyze_password_strength(password):
    ...
    for seq in sequences:
        for i in range(len(seq) - 2):
            frag = seq[i:i+3]
            if frag in password or frag[::-1] in password:
                sequential = True
                break
    if not sequential:
        score += 10
...
def analyze_social_engineering(password, personal_info):
    ...
    for key, value in components.items():
        ...
        # Direct contains (raw and normalized forms)
...
def build_suggestion(password, personal_info):
    ...
    # Remove personal info parts (normalized)
...
def analyze_password():
    ...
    # Penalize if similar to common passwords or uses personal info
    score = base_score
    ...
    advice = [
        ...
        "Prefer passphrases made of unrelated words, then add a number and a symbol.",
...
def generate_password():
    ...
    # Avoid ambiguous characters by default
...
def brute_force_crack(input_password, time_limit=120, personal_info=None):
    ...
    if personal_info:
        for key in ['name', 'date of birth', 'location', 'phone number', 'email username', 'company', 'address']:
            ...
            if v:
                personal_components.append(v.lower())
